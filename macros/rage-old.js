...
const deductResource = false;
const preventNegativeResource = false;
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
const rageMacroName = 'Rage (Game Mechanics)';
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
const warnMacroNotFound =
  ' is not a valid macro name, please fix. Rage toggle successful but unable to alter macro.';
const errorSelectToken = 'Please select a token.';
const errorFailRevert =
  'Failed to revert global melee weapon attack bonus, please check manually.';
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
if (macroActor !== undefined && macroActor !== null) {
  ...
  if (barb !== undefined && barb !== null) {
    ...
    if (
      macroActor.data.flags.rageMacro !== null &&
      macroActor.data.flags.rageMacro !== undefined
    ) {
      enabled = true;
    ...
    let dmg = JSON.parse(
      JSON.stringify(macroActor.data.data.bonuses.mwak.damage),
    );
    ...
    if (enabled) {
      ...
      if (rageDmgAdded) {
        if (
          ...
          if (result !== -1) {
            ...
            let lastHalfDmg = duplicate(dmg).substring(
              result + 3 + len,
              origDmg.length,
            );
      ...
      if (deductResource) {
        ...
        let resourceKey = Object.keys(macroActor.data.data.resources)
          .filter(
            (k) =>
              macroActor.data.data.resources[k].label === `${rageResourceName}`,
          )
          .shift();
        if (
          resourceKey &&
          (macroActor.data.data.resources[resourceKey].value > 0 ||
            !preventNegativeResource)
        ) {
      ...
      if (!noRage) {
        ...
        obj['flags.rageMacro.oldResistances'] = JSON.parse(
          JSON.stringify(macroActor.data.data.traits.dr),
        );
        // add bludgeoning, piercing and slashing resistance
        let newResistance = duplicate(macroActor.data.data.traits.dr);
        if (newResistance.value.indexOf('bludgeoning') === -1)
          newResistance.value.push('bludgeoning');
        if (newResistance.value.indexOf('piercing') === -1)
          newResistance.value.push('piercing');
        if (newResistance.value.indexOf('slashing') === -1)
          newResistance.value.push('slashing');
        //If bear totem, add bear totem resistances.
        bear = macroActor.items.find(
          (i) => i.name === `${bearTotemFeatureName}`,
        );
        if (bear !== undefined && bear !== null) {
          if (newResistance.value.indexOf('acid') === -1)
            newResistance.value.push('acid');
          if (newResistance.value.indexOf('cold') === -1)
            newResistance.value.push('cold');
          if (newResistance.value.indexOf('fire') === -1)
            newResistance.value.push('fire');
          if (newResistance.value.indexOf('force') === -1)
            newResistance.value.push('force');
          if (newResistance.value.indexOf('lightning') === -1)
            newResistance.value.push('lightning');
          if (newResistance.value.indexOf('necrotic') === -1)
            newResistance.value.push('necrotic');
          if (newResistance.value.indexOf('poison') === -1)
            newResistance.value.push('poison');
          if (newResistance.value.indexOf('radiant') === -1)
            newResistance.value.push('radiant');
          if (newResistance.value.indexOf('thunder') === -1)
            newResistance.value.push('thunder');
        }
    ...
    if (!noRage) {
      ...
      if (
        ...
        toggleMacro == true &&
        +(
          stopRageIconPath !== null &&
          stopRageIconPath !== undefined &&
          stopRageIconPath !== ''
        )
      ) {
        if (enabled) {
        ...
        if (toggleMacro == true)
          ui.notifications.warn(`${rageMacroName} ${warnMacroNotFound}`);
      }
    }
  }
